...
Stomatal pores control both leaf gas exchange and are an entry for many plant pathogens, setting up the potential for tradeoffs between photosynthesis and defense. To prevent colonization, plants close their stomata after recognizing pathogens to limit infection. Anatmoical shifts to lower stomatal density and/or size may also limit pathogen colonization, but would permanently reduce the gas exchange capacity for the life of a leaf. I developed and analyzed a spatially explicit model of pathogen colonization on the leaf as function of stomatal size and density. The model predicts that changes in stomatal density or size increase the probability of host plant colonization most when stomatal cover is low. I derived scaling relationships between stomatal size and density that preserves a given probability of colonization. These scaling relationships set up a potential conflict between maximizing defense and minimizing stomatal cover. To my knowledge, this is first mathematical model connecting gas exchange and pathogen defense via stomatal anatomy. It makes predictions that can be tested with experiments and may explain variation in stomatal anatomy among plants. The model is generalizable to many types of pathogens, but lacks significant biological realism that may need to be added in future extensions.
...
In this section, I introduce a spatially explicity, probabilistic model of pathogen colonization on a leaf surface. I explain the model structure and assumptions. The [Methods] section below describes how I analyzed the model to address the goals of the study. I used Sympy version 1.3 [@meurer_sympy:_2017] for symbolic derivations. For generality, I refer to a generic ``pathogen'' that lands on leaf and moves to a stomate. The model is agnostic to the type of pathogen (virus, bacterium, fungus, etc.) and the specific biological details of how it moves (biotrophy). For example, motile bacterial cells can land and move around where fungi may germinate from a cyst and grow until they form an appresorium for infection. These very different biotropic movements on the leaf are treated identically here.
...
--------------------|-----------|-----------------------|-----------------------
 ...
 $L$                | `L`       | $\mu\textrm{m}$       | guard cell length ($L = 2 R$)
...
\begin{figure}
  \centering
	...
	\caption{\textbf{A spatially explicit model of stomatal anatomy and pathogen colonization}. \textbf{a}. Stomata are assumed to be in a homogenous equilateral triangular grid, which means that we can extrapolate from \textbf{b.} a focal triangle to the entire leaf. The circle indicate idealized stomata; the grey lines between them are for visualization. \textbf{c.} By symmetry, a single focal region within the focal triangle can be modeled and extrapolated to the rest of the area. \textbf{d.} The model assumes that a pathogen depicted as a grey rod, lands somewhere on the leaf surface and will sucessfully locate a stomate if it moves in a certain angle, depicted by the grey polygons.}
	\label{fig:fig1}
\end{figure}

## Spatial representation of pathogen search {-}

Now imagine that a pathogen lands at a random position on the leaf and must arrive at a stomate to colonize. If it lands on a stomate, then it infects the leaf with probability 1; if it lands between stomata, then it infects the leaf with probability $p_\text{locate}$. This is the probability that it locates a stomate, which I will derive below. The probabilities of landing on and between a stomate are \fs{} and $1 - $ \fs{}, respectively. Hence, the total probability of infection is:
...
To obtain the average $p_\text{locate}$, we must integrate $f_\text{locate}(x_p,y_p)$ over all possible starting positions $(x_p,y_p)$ within the focal area. The focal area is a 30-60-90 triangle with vertices at the reference stomate $(0,0)$, the midpoint of baseline $(U/2,0)$, and the centroid of the focal trinagle $(U/2, \sqrt{3} / 6 U)$ (Figure \ref{fig:fig1}c). Colonization occurs if the pathogen lands in the reference stomate, so we need to integrate from the edge of this stomate, from $\sqrt{3} / 2 R$ to $U/2$ (Figure \ref{fig:fig2}b). At any $x$, we integrate from the bottom of the focal area ($y_\text{min}$) to the top ($y_\text{max}$):
...
$$ d = c_g - 0.5 s $$
where lowercase variables are log-transformed equivalents of their uppercase counterparts. To preseve $C_g$, the scaling exponent $\beta_g = 0.5$.
...
**Table S1** | Glossary of symbols only used in \gsmax{} calculations.

 Symbol | R      | Units  | Description
--------|--------|--------|-----------------------------------------------------
 $D_w$              | `D_w`    | $\textrm{m}^{2}~\textrm{s}^{-1}$ | diffusion coefficient for water vapour in air at $T_\mathrm{leaf}$ and $P$
 $D_{w,0}$          | `D_w0`   | $\textrm{m}^{2}~\textrm{s}^{-1}$ | diffusion coefficient for water vapour in air at 0 $^\circ$ C and air pressure 101.3246 kPa
 $eT$               | `eT`     | none | stomatal cover ()
 $P$                | `P`      | kPa | atmospheric pressure
 $T_\mathrm{leaf}$  | `T_leaf` | leaf temperature | 
